...
namespace EngineeringOnDisplay2017.Models
{
    public class NaturalGasRepository : INaturalGasRepository
    {
        ...
        public NaturalGasRepository(AppDbContext appDbContext)
        {
        ...
        public IEnumerable<NaturalGasRecord> GetNaturalGasRecords()
        {
            //if no building specified
            if (CurrentBuilding == null)
            {
                //return the entire table of records
                return _appDbContext.NaturalGasRecords.ToList();
            }

            //else return list of records with currentbuilding specified
            return _appDbContext.NaturalGasRecords.Where(record => record.BuildingRecord == CurrentBuilding).ToList();
        }

        //get all records in timeframe with or without concer for the current building
        public IEnumerable<NaturalGasRecord> GetNaturalGasRecords(DateTime start, DateTime end)
        {
            //return a list of records within or equal to the timeframe without concern for current building.
            if (CurrentBuilding == null)
            {
                return _appDbContext.NaturalGasRecords.Where(record => record.RecordedDateTime >= start && record.RecordedDateTime <= end).ToList();
            }

            //else return a list of records within or equal to the timeframe with concern for current building.
            return _appDbContext.NaturalGasRecords.Where(record => record.RecordedDateTime >= start && record.RecordedDateTime <= end && record.BuildingRecord == CurrentBuilding).ToList();
        }
