    ...
    public class ElectricalRepository : IElectricalRepository
    {
        ...
        public IEnumerable<ElectricalRecord> GetElectricalRecords()
        {
            ...
            if (CurrentBuilding == null)
            {
                ...
                return _appDbContext.ElectricalRecords.ToList();
            }

            //else return list of records with currentbuilding specified
            return _appDbContext.ElectricalRecords.Where(record => record.BuildingRecord == CurrentBuilding).ToList();
        }
        ...
        public IEnumerable<ElectricalRecord> GetElectricalRecords(DateTime start, DateTime end)
        {
            ...
            if (CurrentBuilding == null)
            {
                return _appDbContext.ElectricalRecords.Where(record => record.RecordedDateTime >= start && record.RecordedDateTime <= end).ToList();
            }
        ...
        {
            return _appDbContext.ElectricalRecords.FirstOrDefault(record => record.ElectricalRecordId == recordId);
        }
